...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['name_btn'])) { //users send mail range and time update
            $action_std = true;
            $action_cat = 'name';
            ...
            if(!empty($fname) && !empty($lname) && !empty($user_id)){
              $data_update_sql = "UPDATE users SET fname = '$fname', lname = '$lname' WHERE id = '$user_id' ";
              $data_update_read = $db->update($data_update_sql);
              if ($data_update_read) {
                $error = false;
          ...
          if(!empty($img_url)){
            ...
            if (! file_exists($_FILES["img_url"]["tmp_name"])) {
              ...
              $message = "Image size exceeds 500KB";
                 ...
                 $users_read = $db->select($users_query);
                 if ($users_read) {
                     $count_job = mysqli_num_rows($users_read);
                     if($count_job > 0){
                       $worker_row = $users_read->fetch_assoc();
                       ...
                       if (move_uploaded_file($_FILES["img_url"]["tmp_name"], $target)) {
                         $user_update_sql = "UPDATE users SET image = '$img_url' WHERE id = '$user_id' ";
          ...
          if(strlen($new_pass) < 6){
            ...
            $pass_check_sql = "SELECT * FROM users WHERE id = '$user_id' AND password = '$current_pass'";
            $pass_check_read = $db->select($pass_check_sql);
            if ($pass_check_sql) {
              $count_user = mysqli_num_rows($pass_check_read);
              if($count_user > 0){
                $new_pass = md5($new_pass);
                $user_update_sql = "UPDATE users SET password = '$new_pass' WHERE id = '$user_id' ";
                $user_insert_read = $db -> update($user_update_sql);
                if ($user_insert_read) {
                  $error = false;
          ...
          if(!empty($user_id) && !empty($email)){
            $same_email_check_sql = "SELECT * FROM users WHERE id = '$user_id' AND email = '$email'";
            ...
            if ($same_email_check_read) {
              ...
              if($same_email_check_count <= 0){
                $exits_email_check_sql = "SELECT * FROM users WHERE email = '$email'";
                ...
                if ($exits_email_check_read) {
                  ...
                  if($exits_email_check_count <= 0){
                    $check_request_mail_sql = "SELECT * FROM users WHERE email = '$email'";
                    ...
                    if ($check_request_mail_read) {
                      ...
                      if($check_request_mail_count <= 0){
                        $exits_req_check_sql = "SELECT * FROM users WHERE id = '$user_id'";
                        ...
                        if ($exits_req_check_read) {
                          ...
                          if($exits_req_check_count <= 0){
                            $isert_email_request_sql = "UPDATE users SET email_change_token = '$token' WHERE id = '$user_id'";
                            $isert_email_request_read = $db->update($isert_email_request_sql);
                            ...
                            $user_update_sql = "UPDATE users SET email_change_token = '$token' WHERE id = '$user_id' ";
                            $user_insert_read = $db->update($user_update_sql);
                            if ($user_insert_read) {
                              mailsend($email,$token,'update_email');
                              $error = false;
                ...
                $req_delete_sql = "UPDATE users SET email_change_token = '' WHERE id = '$user_id'";
                $req_delete_read = $db->update($req_delete_sql);
                if ($req_delete_read) {
                  $error = false;
          ...
          if(!empty($user_id)){
            ...
            if ($exits_req_check_read) {
              ...
              if($exits_req_check_count > 0){
                ...
                $req_token = $req_data['email_change_token'];
        ...
        if($action_std && !empty($action_cat)){
          ...
          header("Location: ../public/".$profile_page);
        }else{
          header("Location: ../public/".$error_404_page);
        }
      }else{
