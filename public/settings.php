...
$sql_w_l = ! empty( $get_default_type ) ? "SELECT * FROM email_category WHERE user_id = 'all'" : "SELECT * FROM email_category WHERE user_id = '$user_id'";
      ...
      <div class="col-12">
        ...
        <?php } ?>
          <form  action="../functions/<?php echo $settings_func;?>" method="post" enctype="multipart/form-data">
            ...
            <button type="submit" name="site_options_change" class="btn btn-sm btn-info">Save</button>
          </form>
      </div>
      ...
      <div class="col-md-6">
        <div class="card shadow p-4 ml-1 mt-3 mb-3">
          ...
          <?php if(isset($_SESSION['action']) && isset($_SESSION['action_cat']) && $_SESSION['action_cat'] == 'estimated_cost'){ ?>
            <span class="text-<?php echo $_SESSION['action'] ? 'danger' : 'success' ?>"><?php echo isset($_SESSION['action_message']) ? $_SESSION['action_message'] : '' ;?></span>
          <?php } ?>
          <h5 class="">Cost Settings</h5>
          <div class="setting-box mt-4">
            <form action="../functions/<?php echo $settings_func;?>" method="post">
                <div class="form-group row">
                  <label class="col-sm-4 col-form-label">Estimated Cost Saved Per Email</label>
                  <div class="col-sm-8">
                    <input type="text" required name="estimated_cost" class="form-control" value="<?php echo (!empty($d_estimated_cost) ? $d_estimated_cost: '');?>" placeholder="enter your estimated cost value (default $ 0.005)">
                  </div>
                </div>
                <div class="form-group row">
                  <label class="col-sm-4 col-form-label">Cost Per Address Scan</label>
                  <div class="col-sm-8">
                    <input type="text" required name="cost_per_scan" class="form-control" value="<?php echo (!empty($cost_per_scan) ? $cost_per_scan: '');?>" placeholder="enter your cost per scan value (default $ 0.005)">
                  </div>
                </div>
                <div class="form-group">
                  <div class="float-right">
