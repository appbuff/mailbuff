...
function mailsend($username,$mail,$resettoken){
  ...
  $message .= "<p>Mailbuff has received a request to reset the password for your account.</p>";
  $message .= "<p>If you did not request to reset your password, please ignore this email.</p>";
  $message .= "<p>For Reset Password</p>";
  $link = $domain_protocol."://".$domain."/public/".$forgot_password_page."?resettoken=".$resettoken."&email=".$mail;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if(isset($_POST['resetpass-btn'])){
    ...
    if ($read != false) {
      ...
      if ($row > 0) {
        ...
        $check_token_query = "SELECT * FROM users WHERE email = '$email'";
        $check_token_read = $db->select($check_token_query);
        if ($check_token_read != false) {
          $token_row = mysqli_num_rows($check_token_read);
          if($token_row >0 ){
            $token_update_query = "UPDATE users SET reset_pass_token = '$reset_token' WHERE email = '$email' ";
    ...
    $query = "SELECT * FROM users WHERE email = '$email' AND id = '$user_id' AND reset_pass_token = '$token' ";
    $read = $db->select($query);
    $reset_error = true;
    ...
    if($new_pass == $con_pass){
      if ($read != false) {
        ...
        $row = mysqli_num_rows($read);
        if ($row > 0) {
          $check_token_query = "SELECT * FROM users WHERE id = '$user_id'";
          $check_token_read = $db->select($check_token_query);
          if ($check_token_read != false) {
            $token_row = mysqli_num_rows($check_token_read);
            if($token_row >0 ){
              $new_pass = md5($new_pass);
              $token_update_query = "UPDATE users SET password = '$new_pass' WHERE id = '$user_id' ";
              $token_update_read = $db->update($token_update_query);
              if ($token_update_read) {
                  $reset_error = false;
  ...
  $confirm_token_email = "SELECT * FROM users WHERE email = '$confirm_email' AND reset_pass_token = '$confirm_token' ";
...
<head>
  ...
  <title>Forgot Password - Mailbuff</title>
            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="p-5">
                  ...
                  <?php }else{ ?>
                    <div class="text-center">
                      <h1 class="h4 text-gray-900 mb-2 text-uppercase">Forgot Your Password?</h1>
                      <p class="mb-4">Enter your registered email address below and Mailbuff will send you a link to reset your password!</p>
                    </div>
                    <form class="user" action="<?php echo $forgot_password_page;?>" method="post">
                      <div class="form-group">
